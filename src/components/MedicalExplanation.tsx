...
export const MedicalExplanation: React.FC<MedicalExplanationProps> = ({ 
  ...
  const generateExplanation = async () => {
    ...
    try {
      // Try to get analysis ID from results (would need to pass this down as prop)
      // For now, we'll call the medical explanation API with the analysis data
      const response = await fetch('http://localhost:8000/api/medical-explanation/mock', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const mockExplanation = await generateMockExplanation();
  ...
  const generateMockExplanation = async () => {
    return `Based on the blood cell analysis results, here are the key medical insights:
